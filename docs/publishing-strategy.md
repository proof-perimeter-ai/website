...
| `/blog` | **Live**, MDX-powered, built and shipped since this strategy was first drafted (see §4 for the implementation). **13 published posts** so far, spanning all four Phase 1 pillars (`kyc-document-automation`, `ai-bank-statement-analysis-loan-underwriting`, `claims-processing-document-ai`, `sovereign-ai-gap`), the OCR-fundamentals explainer (`what-is-ocr-ai`), all six Phase 2/Cluster C fast-win posts (`agentic-document-extraction`, `zero-shot-vs-fine-tuned-document-extraction`, `document-classification-ai`, `confidence-scoring-document-extraction`, `deep-extraction-vs-shallow-ocr`, `agentic-document-workflow-orchestration`, `schema-based-document-extraction` — see §8 for the full per-post detail), and the first Cluster B post, "What Is Google Document AI? How It Works, Strengths, and Weaknesses" (`content/blog/what-is-google-document-ai.mdx`, 2026-07-31) — a vendor-neutral explainer, not a named comparison; see the 2026-08-01 operator directive in §8's Cluster B section pausing "X vs. Y" comparison titles. **All four Phase 1 pillars and Cluster C are done; Cluster B is open for educational/explainer framing only until that directive lifts**, and both vendor-neutral explainers named in this section as of 2026-07-31 (`what-is-google-document-ai`, `what-is-azure-ai-document-intelligence`, published 2026-08-01) are now live — see §8 for the full Cluster B backlog, including remaining unmeasured-keyword and topic candidates ("ocr ai" — already substantially covered by the published `what-is-ocr-ai` pillar per the routine's Step 1 inventory check, despite being unchecked in §8 — plus the still-unchecked topic candidates below it; the named comparison titles stay unchecked and paused). Every published post folds the Proof Perimeter pitch into a body section rather than a bolted-on close. |
