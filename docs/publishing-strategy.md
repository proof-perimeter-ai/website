...
| `/blog` | **Live**, MDX-powered, built and shipped since this strategy was first drafted (see §4 for the implementation). **13 published posts** so far, spanning all four Phase 1 pillars (`kyc-document-automation`, `ai-bank-statement-analysis-loan-underwriting`, `claims-processing-document-ai`, `sovereign-ai-gap`), the OCR-fundamentals explainer (`what-is-ocr-ai`), all six Phase 2/Cluster C fast-win posts (`agentic-document-extraction`, `zero-shot-vs-fine-tuned-document-extraction`, `document-classification-ai`, `confidence-scoring-document-extraction`, `deep-extraction-vs-shallow-ocr`, `agentic-document-workflow-orchestration`, `schema-based-document-extraction` — see §8 for the full per-post detail), and the first Phase 3/Cluster B comparison post, "Proof Perimeter vs. Google Document AI: Which Fits Regulated BFSI Workflows?" (`content/blog/proof-perimeter-vs-google-document-ai.mdx`, 2026-07-31). **All four Phase 1 pillars and Cluster C are done; Phase 3 (Cluster B comparison content) is now open** — see §8 for the remaining unchecked Cluster B candidates ("ocr ai", "azure ai document intelligence", and the Azure/Amazon Textract/on-premise-alternatives comparison titles). Every published post folds the Proof Perimeter pitch into a body section rather than a bolted-on close. |
...
- [ ] ocr ai
- [x] google document ai — published as `proof-perimeter-vs-google-document-ai` (2026-07-31), first Phase 3 post (Cluster B comparison content), opening the phase now that Cluster C closed at 6 published posts (see §1/§5); primary keyword "google document ai" (UK vol 260, KD 28, intent_fit 2 — category/comparison per §6 — priority 136.8, highest-scoring unchecked Cluster B candidate, ahead of "ocr ai" at 85.2 and "azure ai document intelligence" at 60.7); operationalized via the matching title candidate below, which targets the same keyword with the named-comparison framing
- [ ] azure ai document intelligence
- [x] Proof Perimeter vs. Google Document AI: Which Fits Regulated BFSI Workflows? — published as `proof-perimeter-vs-google-document-ai` (2026-07-31); same post as the `google document ai` keyword row above — the title candidate is how that keyword got operationalized this cycle; perspective angle is a named, specific comparison (Proof Perimeter vs. Google Document AI, not "cloud vs. on-prem" in the abstract) plus sourced G2 (4.2/5, 36 reviews, pricing/accuracy complaints) and Gartner Peer Insights buyer-pain findings, the Gartner Sept 2025 IDP Magic Quadrant's Challenger (not Leader) placement for Google, and a primary-source fact from Google's own regions documentation (nine total fixed GCP locations, no on-premise/VPC path); retroactively added to `sovereign-ai-gap`'s relatedPosts (replacing its `claims-processing-document-ai` slot, which remains reachable via shared tags) plus one contextual in-body link
