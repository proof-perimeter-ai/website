...
| `/blog` | **Live**, MDX-powered, built and shipped since this strategy was first drafted (see §4 for the implementation). **3 published posts** so far: "What Is OCR AI?" (`content/blog/what-is-ocr-ai.mdx`) — an educational explainer sitting in Cluster A/G territory (head-term "document ai"/"ocr ai" education, OCR-fundamentals framing) — "KYC Document Automation: From Manual Review to AI-Assisted Onboarding" (`content/blog/kyc-document-automation.mdx`) — the first Phase 1 pillar page (Cluster A/E), anchoring "KYC & Onboarding Document AI" — and "AI Bank Statement Analysis for Loan Underwriting: Automating Income Verification" (`content/blog/ai-bank-statement-analysis-loan-underwriting.mdx`) — the second Phase 1 pillar page (Cluster D), anchoring "Sovereign AI for Lending Document Processing". All three fold the Proof Perimeter pitch into a body section rather than a bolted-on close. 2 of 4 Phase 1 pillars and all comparison/regulatory content remain unpublished. |
