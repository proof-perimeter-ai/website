...
relatedPosts: ["sovereign-ai-gap", "document-classification-ai", "claims-processing-document-ai"]
status: "published"
...
The first step in any automated KYC pipeline is figuring out what a given file even is — a passport, a utility bill, a shareholder register — often without a human ever specifying the document type up front. [Document classification AI](/blog/document-classification-ai) sorts incoming pages by document type and routes each one to the extraction logic built for it, since a passport and a bank statement need entirely different fields pulled from entirely different layouts.
...
This is the specific gap Proof Perimeter is built to close. Its fine-tuned document AI models run inside a bank or fintech's own environment — cloud-hosted, within customer infrastructure, or fully on-premise on commodity CPUs — so passports, proof-of-address documents, and beneficial-ownership filings never have to leave the institution's own perimeter to be read. On Proof Perimeter's internal benchmarks, that fine-tuned model delivers 20% higher accuracy and 50% lower token consumption than general-purpose frontier models on the same document-extraction tasks, with field-level provenance attached to every extracted value — the "what did the model see, and why did it decide this" record a KYC file needs to survive an examiner's questions, not just an onboarding SLA. This distinction — between where a document is stored and where the model actually reads it — is exactly what we mean by the [sovereign AI gap](/blog/sovereign-ai-gap), and it's worth understanding on its own terms before it shows up in a vendor review.
