...
relatedPosts: ["zero-shot-vs-fine-tuned-document-extraction", "sovereign-ai-gap", "document-classification-ai"]
status: "published"
...
The first step is recognizing what's actually in the file — which pages belong to which account, which bank issued the statement, and which format variant it follows — before any extraction logic runs. Because format diversity is the core difficulty in this document type, [document classification](/blog/document-classification-ai) has to generalize across bank-specific layouts rather than relying on a template built for one issuer. This is also exactly where the underlying model matters most: a model reading an issuer's statement layout for the first time is working [zero-shot](/blog/zero-shot-vs-fine-tuned-document-extraction), while one that's seen enough of a lender's actual statement mix has been fine-tuned specifically to it — and the accuracy gap between those two states is largest on precisely the long-tail formats a generic pipeline sees least often.
